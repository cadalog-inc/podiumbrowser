...
export class SubCategories extends React.Component {
    render() {
        let items = (this.props.isHomeCategory(this.props.category.id) && (this.props.query.searchTerm === "") ? this.props.items.filter((item) => {
            return this.props.searchArray(item.tags, this.props.query.searchTerm) && (this.props.useHDR || item.filename.split('.')[1] !== 'hdr');
